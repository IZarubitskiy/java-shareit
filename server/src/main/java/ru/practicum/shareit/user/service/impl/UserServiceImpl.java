...
public class UserServiceImpl implements UserService {
    ...
    public UserDtoResponse add(UserDtoRequestCreate userDtoRequestCreate) {
        ...
        if (userRepository.existsByEmail(userDtoRequestCreate.getEmail())) {
            throw new DuplicationException(
                    String.format("User with email %s already exists", userDtoRequestCreate.getEmail())
            );
        }
    ...
    public UserDtoResponse update(Long userId, UserDtoRequestUpdate userDtoRequestUpdate) {
        ...
        if (userRepository.existsByEmail(userDtoRequestUpdate.getEmail())) {
            throw new DuplicationException(
                    String.format("User with email %s already exists", userDtoRequestUpdate.getEmail())
            );
        }
